...
Deploy a highly available 2-tier AWS architecture (Web Tier + Database
Tier), fully automated with Infrastructure as Code (Terraform) and a
CI/CD pipeline (Jenkins), using remote state management (S3 +
DynamoDB) for team-safe, locked Terraform operations.
...
- **Internet Gateway** attached to the VPC, routed from the public
  route table only
- **2 EC2 instances** (Apache) in the public subnets, each behind its
  own security group allowing HTTP (80) and SSH (22)
- **1 RDS MySQL instance** (Multi-AZ) in the private subnets, reachable
  only on port 3306 from the web tier's security group — not from the
  public internet
- **Terraform remote state** stored in an S3 bucket (versioned) with a
  DynamoDB table for state locking, preventing concurrent
...
- Policies attached: `AmazonEC2FullAccess`, `AmazonVPCFullAccess`,
  `AmazonRDSFullAccess`, `AmazonS3FullAccess`,
  `AmazonDynamoDBFullAccess`, `IAMReadOnlyAccess`
...
- `security_groups.tf` — `web-sg` (80/22 from anywhere) and `rds-sg`
...
Resources created (18 total, per `terraform plan`):
VPC, Internet Gateway, 4 subnets, 2 route tables, 4 route table
associations, 2 security groups, 2 EC2 instances, 1 DB subnet group, 1
RDS instance.
...
- Security group allows inbound SSH (22) and Jenkins UI (8080) from the
  administrator's IP
...
- Definition: **Pipeline script from SCM** (Git), reading `Jenkinsfile`
  from the repo root, branch `main`

### Pipeline Stages (`Jenkinsfile`)
1. **Checkout Code** — pulls the latest commit from GitHub
2. **Setup Terraform Environment** — confirms `terraform -version`
3. **Terraform Init** — connects to the S3/DynamoDB backend, installs providers
4. **Terraform Plan** — generates and saves an execution plan (`tfplan`)
5. **Approval** — pipeline pauses (`input` step) until a human clicks Apply
6. **Terraform Apply** — applies the saved plan, provisioning all resources
...
_Add these to `docs/screenshots/` and reference them here:_
...
2. Delete the `jenkins-sg` security group
3. Empty and delete the S3 state bucket
4. Delete the DynamoDB `terraform-locks` table
5. Delete the `terraform-jenkins-user` IAM user (and its access keys)

## 11. Lessons Learned

- Terraform's S3 backend requires the bucket name to match **exactly**
  — even a small naming inconsistency (e.g. `tfstate` vs
  `terraform-state`) causes `terraform init` to fail outright.
- A manual approval gate (`input` step) in the Jenkins pipeline is a
  simple but effective safeguard against accidentally applying
  infrastructure changes without review.
- Keeping the database tier in private subnets with no internet route,
  and restricting its security group to only accept traffic from the
  web tier's security group (rather than a CIDR range), is a stronger
  and more maintainable isolation pattern than IP-based rules.
- `skip_final_snapshot = true` on the RDS instance was a deliberate
  choice for this learning project to make `terraform destroy` fast and
  clean; in production this would be set to `false` to protect against
